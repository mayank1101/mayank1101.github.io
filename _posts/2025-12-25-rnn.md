...
series_image: "/assets/images/2025-12-25-rnn/rnn-architecture.png"
...
7. [Building an RNN from Scratch](#building-an-rnn-from-scratch)
...
- The loss at time $t$ (if there's an output)
- The gradient from the next time step $t+1$
...
- Process the sequence in chunks (e.g., 50 time steps)
- Backpropagate only through the chunk
- Pass the hidden state forward to the next chunk (but don't backpropagate through it)
...
For hands-on practice, check out the companion notebooks - [Understanding RNN Architecture From Scratch](https://drive.google.com/file/d/1P7fFok2Vp9ZsyqO6UQagrV6V16Sxv4Jt/view?usp=sharing)