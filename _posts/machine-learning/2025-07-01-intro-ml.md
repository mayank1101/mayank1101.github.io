...
# [grocery, electronics, clothing] spending
customers = [[100, 20, 5],   
             [95, 25, 10],
             [30, 200, 150],
             [25, 180, 200]]
...
Ensures each split has same proportion of each class. If 70% class A, 30% class B in full data, then stratified split maintains 70%-30% in train and test.
...
A single train-test split can be "unlucky" sometimes, what if your test set happens to be particularly easy or hard? Cross-validation solves this by testing on multiple different splits.
...
It is one of the most important concepts in machine learning and this tradeoff applies to all types of machine learning models. It is important to understand this tradeoff in order to build effective machine learning models.

### Understanding Bias and Variance

**Bias**: Error that occurs when a model makes assumptions that are too simple. Instead of truly learning the patterns in the data, the model forces a rigid idea onto it. This oversimplification causes the model to make consistently wrong predictions (known as underfitting). High bias can result from using an overly simple model (like a linear model for non-linear data), insufficient training data, or inappropriate feature selection.

- High bias → Model is too simple
- Underfits the data
- Doesn't capture underlying patterns
- Leads to high error on both training and testing data

**Variance**: Error that occurs when a model is too sensitive to small fluctuations in training data. Instead of learning the true underlying patterns, the model memorizes the training data, including its noise and random variations. This over-sensitivity causes the model to perform well on the training data but poorly on new, unseen data (known as overfitting). High variance can result from using an overly complex model (like a high-degree polynomial for simple data), insufficient training data, or inappropriate feature selection.

- High variance → Model is too complex
- Overfits the data
- Captures noise as if it were a pattern
- Leads to low error on training data but high error on testing data

### The Tradeoff

Let's try to understand the bias-variance tradeoff with an example. Suppose you have a dog and you want to train it to fetch a ball. 

**1. High Bias (Underfitting)** 
You throw a yellow tennis ball. Your dog fetches it. Next, you throw a red apple. The dog ignores it completely because it only looks for yellow, fuzzy spheres.

**What happened?** The dog made an overly simple rule: "Only yellow fuzzy things are fetchable". In Machine Learning this is High Bias (or Underfitting). Similarly, a model with high bias makes assumptions that are too simple. It misses the big picture and performs poorly on both your practice data and new data. 

**2. High Variance (Overfitting)** 
You throw the yellow tennis ball on a Tuesday afternoon while wearing a blue hat. The dog fetches it. Next, you throw the same ball on a Wednesday while wearing a green hat. The dog sits still and ignores it. 

**What happened?** The dog memorized every single detail of the first throw. It thinks fetching only works on Tuesdays when you wear a blue hat. In Machine Learning, this is High Variance (or Overfitting). Your model memorizes the practice data too perfectly, including random noise and useless details. It looks amazing during practice, but fails completely in the real world. 

**3. Sweet Spot** 
You want a dog that understands the core concept of fetching: go grab the object you threw, regardless of its color or the color of your hat. This is the **Sweet Spot**, where the model generalizes well to new data without being too simple or too complex.


### Managing the Tradeoff

**Reduce Bias** (if model is underfitting)
...
**Reduce Variance** (if model is overfitting):
...
Let's try to understand the bias-variance tradeoff mathematically. Consider we are fitting a polynomial to data:
...
**Definition**: When a model is too simple to capture the underlying patterns in the data, it is said to be underfitting. In other words, the model makes strong assumptions about the data and fails to capture the true relationship between the features and the target variable. This leads to high error on both training and testing data.
...
Let's say we want to predict house prices. In reality, house prices are affected by many factors, such as size, location, age, number of bedrooms, etc. However, we decide to build a model that predicts house prices based only on their size.
...
**Definition**: When a model try to memorize the training data instead of learning general patterns, it is said to be overfitting.
...
Let's say model training accuracy is 99% but test accuracy is 65%, it means the model is overfitting. The model learned the training data too well, including the noise and outliers, but failed to generalize to new data.
...
**The Goldilocks Principle** is to find model that is "just right" i.e the model is complex enough to capture true patterns and simple enough to generalize to new data. It is the sweet spot between underfitting and overfitting. In a real-world scenario, the best approach is to start with a simple model and gradually increase its complexity until you achieve the desired performance.
...
Now that you understand the fundamental concepts that underpin all of machine learning, you're ready to dive deeper into specific algorithms and techniques. In the next article we will learn **Linear Regression**.
...
**Books:**
- ["Hands-On Machine Learning" by Aurélien Géron](https://www.amazon.in/Hands-Machine-Learning-Scikit-Learn-TensorFlow/dp/9355421982)
