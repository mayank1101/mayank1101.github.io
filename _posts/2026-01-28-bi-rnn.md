...
9. [Jupyter Notebook](#jupyter-notebook)
...
def demonstrate_bidirectional_vs_unidirectional():
    ...
    for i, (seq_indices, true_label) in enumerate(examples):
        ...
        print(f"   Bidirectional: {bi_sentiment} {'✅' if bi_pred == true_label else '❌'}")
        print(f"   Unidirectional: {uni_sentiment} {'✅' if uni_pred == true_label else '❌'}")
...
- [Hochreiter, S., & Schmidhuber, J. (1997). Long short-term memory. Neural computation, 9(8), 1735-1780](https://deeplearning.cs.cmu.edu/F20/document/readings/Bidirectional%20Recurrent%20Neural%20Networks.pdf)
- [Cho, K., Van Merriënboer, B., Gulcehre, C., Bahdanau, D., Bougares, F., Schwenk, H., & Bengio, Y. (2014). Learning phrase representations using RNN encoder-decoder for statistical machine translation](https://arxiv.org/pdf/1406.1078)
- [Graves, A., & Schmidhuber, J. (2005). Framewise phoneme classification with bidirectional LSTM and other neural network architectures. Neural networks, 18(5-6), 602-610](https://www.cs.toronto.edu/~graves/ijcnn_2005.pdf)
- [Bahdanau, D., Cho, K., & Bengio, Y. (2014). Neural machine translation by jointly learning to align and translate](https://arxiv.org/pdf/1409.0473)

## Jupyter Notebook

For hands-on practice, check out the companion notebooks - [Understanding Bidirectional RNNs with PyTorch](https://drive.google.com/file/d/1J8xB_uGEvEH7eiLqnNZVsIMuu-KUtDne/view?usp=sharing)